      ...
      <div className="h-64 w-full space-y-2">
        <div className="flex items-center justify-end gap-4 text-xs font-mono">
          ...
          <span className="flex items-center gap-1.5 text-purple-400">
            <span className="w-2.5 h-2.5 bg-purple-500/80 inline-block rounded-sm"></span>
            🟣 투자 자산 (신NISA/주식/iDeCo)
          </span>
        </div>
              ...
              <RechartsTooltip
                ...
                formatter={(value: any, name: any) => {
                  const labelMap: Record<string, string> = {
                    displayNetWorth: '총자산 (합계)',
                    displayInvestments: '투자 자산 (NISA/주식/iDeCo)',
                    displayCash: '현금 자산',
                  };
                  return [
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
          {snapshots.map((s) => (
            <div
              key={s.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between bg-zinc-800/40 border border-zinc-800/80 rounded-xl px-3.5 py-2 text-xs group gap-1"
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-emerald-400 font-mono font-bold">
                  총자산 {isPrivate ? '••••' : formatJPYShort(s.netWorth)}
                </span>
                <span className="text-purple-400 font-mono text-[11px]">
                  (투자자산 {isPrivate ? '••••' : formatJPYShort(s.totalInvestments)})
                </span>
                {s.note && <span className="text-zinc-400 text-[11px] truncate max-w-[150px]">({s.note})</span>}
              </div>
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition self-end sm:self-auto">
                <button
