# 🚀 Financial OS · Japan Edition (v1.8.2)
...
- 🏦 **전 계좌 통합 총자산 개념 명확화**: 현금 + 주식 + 신NISA + iDeCo + 부동산 모든 계좌 실시간 전체 합산 수치임을 대시보드 표기
- 🛠️ **금액 입력 필드 UX 개선**: 백스페이스 시 '0'이 지워지지 않던 현상을 해결하여 깔끔하게 빈칸(`""`) 처리
- 🔖 **버전 동적 동기화**: 전역 `APP_VERSION = 'v1.8'` 도입으로 사이드바 및 푸터 버전 동기화
